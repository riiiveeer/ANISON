...
- A/B/C 工作树已在施工分支形成独立基线提交 `bd647a6`；仍需推送并通过远端 CI。
...
- 纯部署配置和文档不改变业务版本，首次 Render 部署继续使用 `1.0.0-beta.3`。
...
- D0：A/B/C 已形成提交 `bd647a6`；`npm ci`、Node 测试（140/140）、生产构建、依赖审计、核心 E2E、性能门禁和 PWA E2E 通过。
...
- D3 尚需推送分支，在 GitHub 执行 Linux CI，并让 `main` 三项必需检查实际生效。
...
验收：关闭开发者电脑后，固定 HTTPS `/healthz` 仍返回 `1.0.0-beta.3` 与正确提交号。
