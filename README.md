...
> 当前状态：`1.0.0-beta.4` 朋友测试版。生产 PWA 与 Render Blueprint 已完成本地施工；固定 HTTPS 服务、真实手机和回滚验收仍须完成后才会公布为可用 Beta。更新前仍建议先在设置页导出完整备份。
...
- IndexedDB v4 聚合本地存储：歌曲摘要、单曲内容文档与按需创建的稀疏学习状态
- schema v3 完整数据导出/恢复，兼容导入 v1/v2，并保留取消、异常和重启自动回滚
...
Windows Chromium 的磁盘型 IndexedDB 吞吐波动较大，本机性能任务默认跳过两项“完整迁移 30 秒”硬门禁；Linux CI 会验证 80,000 个逻辑学习单元在全默认状态时只写 0 条物理状态、10% 非默认状态时只写 8,000 条状态。聚合恢复 `<10 秒` 与其余页面预算仍会在本机执行。
...
v4 的 `songs` 只承担列表和查重摘要，`songContents` 每首歌保存一个聚合内容文档，`learningStates` 只保存已学习、收藏或有复习历史的单元；不存在的状态按默认 `new` 解释。v2、完整 v3 和部分迁移 v3 会在同一个数据库升级事务中转成 v4，失败时浏览器保留旧版本。迁移前逻辑记录暂存于业务不可见的 `migrationArchive`，计划到 v5 再独立清理。
