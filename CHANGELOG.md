...
- IndexedDB v4 原子升级器、迁移归档和备份 schema v3，兼容 v2、完整 v3 与部分迁移 v3 数据。
...
- 本地数据改为“歌曲摘要 + 单曲聚合内容 + 稀疏学习状态”；默认 `new` 状态不落盘，列表、标题编辑和复习查询不再逐卡读取 IndexedDB。
- 1,000 首/80,000 卡性能门禁拆为全默认迁移、10% 状态迁移、聚合恢复和页面性能独立用例，继续执行 `<30 秒`/`<10 秒` 预算。
...
- 修复逐卡 v3 结构使 80,000 卡升级产生过多 IndexedDB 请求、Linux CI 迁移超过三分钟的问题；v4 改为每曲有界转换、延后建索引并在同一 `versionchange` 事务中验证。
